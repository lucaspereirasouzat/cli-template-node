...
import { CreateUseCase } from "../../../src/domain/use-cases/create-use-case";
import {
  ReadFile,
  FolderExists,
  MakeDir,
...
describe("Create UseCase", () => {
  ...
  beforeEach(() => {
    useCase = new CreateUseCase(fileStorage, pathresolve, logger);
  });
  ...
  it("should be able validate if folder exists ", () => {
    ...
    expect(fileStorage.folderExists).toBeCalledWith({ path: "aa/src/domain/use-cases/" });
  });
  it("should be able to create folder ", () => {
    ...
    expect(fileStorage.makeDir).toBeCalledWith({ path: "aa/src/domain/use-cases/" });
  });
});
